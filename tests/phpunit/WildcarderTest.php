...
class WildcarderTest extends MediaWikiIntegrationTestCase {
	...
	public function testWildcardSingleWord() {
		...
		$this->assertEquals( "(test OR *test OR test* OR *test*)", $result );
	}
	...
	public function testWildcardMultiWord() {
		...
		$wildcarder = Wildcarder::factory( $origin );
		$result = $wildcarder->getWildcarded();
		$this->assertEquals( "(Test OR *Test OR Test* OR *Test*) (text OR *text OR text* OR *text*)", $result );
	}
	...
	public function testWildcardMultiWordWithSeparators() {
		...
		$result = $wildcarder->getWildcarded();
		$this->assertEquals(
			"(Test OR *Test OR Test* OR *Test*) (text OR *text OR text* OR *text*) (dummy OR *dummy OR dummy* OR *dummy*)", // phpcs:ignore Generic.Files.LineLength.TooLong
			$result
		);
	}
